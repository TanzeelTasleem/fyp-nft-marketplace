...
import { GetListedNftsInput, GetListedNftsOutput,NftListing,Sorting_Order,BlockChainStatus } from "../codegenTypes";
...
type outputType = ErrorStructure | { data: GetListedNftsOutput }

export async function GetListedNfts(input: GetListedNftsInput, faunaClient: Client):Promise<outputType> {

    console.log("input ", input);
    const {pageSize,after,before,SortingOrderByTime,category,listingType} = input;
    ...
    let faunaMatchArray = []
      ...
      if (SortingOrderByTime === Sorting_Order.Asc){
          ...
          faunaMatchArray.push(q.Match(q.Index("getListedNfts_searchBy_status_sortBy_ts_asc"),BlockChainStatus.Confirmed))
          ...
          faunaMatchArray.push(q.Match(q.Index("getListedNfts_searchBy_status_sortBy_ts_desc"),BlockChainStatus.Confirmed))
   ...
   afterOutput[2] = getListedNfts.after[2].id
    ...
    if (getListedNfts.before){
     ...
     let output:GetListedNftsOutput = {listedNfts:listings}
