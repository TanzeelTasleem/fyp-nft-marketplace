...
export async function getCreateCollectionStatus(faunaClient:Client, input:GetBlockChainStatusInput): Promise<outputType> {
    const {id} = input;
    ...
    try {
        ...
        if (getCollection.data.length>0){
            output = {status:getCollection.data[0].data.blockChainStatus} 
        }
